...
class IssueDetailsCard(issue: OperationOutcomeIssue) : JBScrollPane() {
        ...
        val content =
            JPanel().apply {
                ...
                if (issue.locations.isNotEmpty()) {
                    val components = issue.locations.map { createLocationField(it) }
                ...
                if (issue.line != null || issue.column != null) {
                    val lineColText =
                        listOfNotNull(
                            issue.line?.let { "line $it" },
                            issue.column?.let { "column $it" },
                        ).joinToString(", ").capitalizeWithCurrentLocale()
                    add(
                        stretchHorizontally(
                            createSection(
                                "Source location",
                                createSingleLineField(lineColText),
                            ),
                        ),
                    )
                    add(Box.createVerticalStrut(JBUI.scale(18)))
    ...
    private fun createLocationField(text: String): JComponent =
        JPanel(BorderLayout(JBUI.scale(8), 0)).apply {
            ...
            add(
                JLabel(AllIcons.General.Locate).apply {
                    background = UIUtil.getActiveTextColor()
                },
                BorderLayout.WEST,
            )
            add(
